...
import { MessageSquare, Upload, Trash2, Heart, MessageCircle, Search as SearchIcon, Filter, Image as ImageIcon, Pin, User } from "lucide-react";
 ...
 export default function Community() {
  ...
  const filtered = useMemo(() => {
    ...
    const list = posts
      ...
      .filter((p) => (q ? [p.author || "", p.content, p.topic].join(" ").toLowerCase().includes(q) : true));
  ...
  const submit = (values: FormValues) => {
    if (!values.content || values.content.trim().length < 3) {
      toast({ title: "Post too short", description: "Write at least 3 characters." });
      return;
    }
    const item: PostItem = {
  ...
  const pinPost = (id: string) => setPosts((prev) => prev.map((p) => (p.id === id ? { ...p, pinned: !p.pinned } : p)));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Connect with neighbors, share updates, and ask for help.</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Share an update, ask a question, or post an alert.</CardDescription>
              </CardHeader>
              <CardContent>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(submit)} className="space-y-4">
                    ...
                    <FormField control={form.control} name="content" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Textarea rows={4} placeholder="What's on your mind?" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />

                    <div className="flex items-center justify-end gap-2">
                      <Button type="button" variant="secondary" onClick={() => { form.reset({ ...form.getValues(), content: "" }); setImages([]); }}>
                        Clear
                      </Button>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Filter className="w-5 h-5" /> Feed</CardTitle>
                <CardDescription>Sort and filter community posts.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
                  ...
                  <TabsContent value="new" className="space-y-3 [&[data-state=inactive]]:hidden" />
                  <TabsContent value="top" className="space-y-3 [&[data-state=inactive]]:hidden" />
                </Tabs>
                ...
                <div className="space-y-3 max-h-[640px] overflow-auto pr-1">
                  {filtered.length === 0 && (
                    <div className="text-sm text-muted-foreground">No posts match your filters.</div>
                  )}

                  {filtered.map((p) => (
                    <div key={p.id} className={cn("border border-border rounded-lg p-3 bg-card/60", p.pinned && "ring-1 ring-indigo-300") }>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-medium text-foreground">{p.author || "Anonymous"}</span>
                              ...
                              {p.pinned && <Badge variant="secondary">Pinned</Badge>}
                            </div>
                            <div className="mt-1 text-sm whitespace-pre-wrap text-foreground">{p.content}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button size="icon" variant="ghost" onClick={() => pinPost(p.id)} aria-label="Pin">
                            <Pin className={cn("w-4 h-4", p.pinned ? "text-indigo-600" : "text-muted-foreground")} />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => removePost(p.id)} aria-label="Delete">
                            <Trash2 className="w-4 h-4 text-red-600" />
                          </Button>
                        </div>
                      </div>
                      ...
                      {p.images.length > 0 && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
                          {p.images.slice(0, 4).map((src, i) => (
                            <img key={i} src={src} alt={`post-${i}`} className="w-full aspect-square object-cover rounded" />
                          ))}
                        </div>
                      )}

                      <div className="mt-2 flex items-center gap-3 text-sm">
                        <button onClick={() => toggleLike(p.id)} className="inline-flex items-center gap-1 text-muted-foreground hover:text-foreground">
                          <Heart className={cn("w-4 h-4", user.likes[p.id] ? "fill-current text-red-600" : "")} /> {p.likes}
                        </button>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {trending.map(([t, count]) => (
                  <div key={t} className="flex items-center justify-between border rounded-lg p-2">
                    <div className="flex items-center gap-2">
