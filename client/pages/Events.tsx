...
import { Calendar as CalendarIcon, Upload, Trash2, MapPin, Clock, Users, CheckCircle2, Search } from "lucide-react";
...
import { cn } from "@/lib/utils";
 ...
 export default function Events() {
  ...
  const [userStatus, setUserStatus] = useState<UserStatusMap>(() => readUserStatus());
  ...
  const filtered = useMemo(() => {
    const list = events
      ...
      .filter((e) => {
        ...
        if (tab === "upcoming") return e.status === "upcoming" || e.status === "ongoing";
        return e.status === "completed";
      })
      .filter((e) => {
        const q = query.trim().toLowerCase();
        if (!q) return true;
        return [e.title, e.description, e.location, e.category].join(" ").toLowerCase().includes(q);
      })
  ...
  const onSubmit = (values: FormValues) => {
    if (!values.title || values.title.trim().length < 3) {
      toast({ title: "Title too short", description: "Add a descriptive title (min 3 chars)." });
      return;
    }
    ...
    if (!values.location || values.location.trim().length < 2) {
      toast({ title: "Location required", description: "Provide a clear location or venue." });
      return;
    }
    if (!values.date) {
      toast({ title: "Date required", description: "Pick a date from the calendar." });
      return;
    }

    const startISO = toISO(values.date, values.time || "00:00");
    const endISO = values.endTime ? toISO(values.date, values.endTime) : undefined;
    ...
    setEvents((prev) => {
      const next = [...prev, item].sort((a, b) => new Date(a.dateISO).getTime() - new Date(b.dateISO).getTime());
      return next;
    });
    setImages([]);
    form.reset({
  ...
  const toggleGoing = (id: string) => {
    setUserStatus((prev) => {
      ...
      setEvents((evs) => evs.map((e) => (e.id === id ? { ...e, attendees: e.attendees + (!current ? 1 : -1) } : e)));
      return nextMap;
    });
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Discover and organize events around your community.</p>
          </div>
        </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Share details to get people involved.</CardDescription>
                </CardHeader>
                <CardContent>
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormField control={form.control} name="title" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input placeholder="e.g. Neighborhood Cleanup Drive" {...field} /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )} />
                      ...
                      <FormField control={form.control} name="description" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Textarea rows={4} placeholder="What is this about, agenda, who should join, etc." {...field} /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      ...
                      <FormField control={form.control} name="location" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input placeholder="e.g. IETK Main Auditorium" {...field} /></FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button type="submit"><CheckCircle2 className="w-4 h-4 mr-2" /> Create</Button>
                      </div>
                    </form>
                  </Form>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><Search className="w-5 h-5" /> Browse Events</CardTitle>
                  <CardDescription>Filter by date, category, and keywords.</CardDescription>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="md:col-span-2 space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        <Input placeholder="Search title, location, description" value={query} onChange={(e) => setQuery(e.target.value)} />
                      </div>
                      ...
                      <div className="space-y-3 max-h-[420px] overflow-auto pr-1">
                        ...
                        {filtered.map((e) => (
                          <div key={e.id} className={cn("border rounded-lg p-4", e.status === "cancelled" ? "bg-red-50 border-red-200" : "border-border bg-card/60") }>
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="font-semibold text-foreground">{e.title}</span>
                                  <Badge variant="secondary">{e.category}</Badge>
                                  <Badge variant="secondary">{new Date(e.dateISO).toLocaleString()}</Badge>
                                  {e.status !== "upcoming" && <Badge variant="secondary">{e.status}</Badge>}
                                </div>
                                <div className="text-sm text-muted-foreground flex items-center gap-2 mt-1">
                                  <MapPin className="w-4 h-4" /> {e.location}
                                  <Clock className="w-4 h-4 ml-3" /> {new Date(e.dateISO).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                                </div>
                              </div>
                              <div className="text-right">
                                <div className="flex items-center justify-end gap-2">
                                  <Badge variant="secondary" className="flex items-center gap-1"><Users className="w-3 h-3" /> {e.attendees}</Badge>
                                  <Button size="sm" variant={userStatus[e.id]?.going ? "default" : "secondary"} onClick={() => toggleGoing(e.id)}>
                                    {userStatus[e.id]?.going ? "Going" : "I'm going"}
                                  </Button>
                                  <Button size="icon" variant="ghost" onClick={() => cancelEvent(e.id)} aria-label="Cancel event">
                                    <Trash2 className="w-4 h-4 text-red-600" />
                                  </Button>
                                </div>
                              </div>
                            ...
                            {e.photos.length > 0 && (
                              <div className="grid grid-cols-4 gap-2 mt-3">
                                {e.photos.slice(0, 4).map((src, i) => (
                                  <img key={i} src={src} alt={`event-${i}`} className="w-full aspect-square object-cover rounded" />
                                ))}
                              </div>
                            )}

                            <div className="text-sm text-muted-foreground mt-2 whitespace-pre-wrap">{e.description}</div>

                            {e.status === "cancelled" && (
                              <div className="mt-2 text-sm text-red-600">This event has been cancelled.</div>
                            )}
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              <Button size="sm" variant="outline" onClick={() => removeEvent(e.id)}>
                                Delete
                              </Button>
                            </div>
                          </div>
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Help attendees plan and participate.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <div>Include start/end times and exact location details.</div>
                  <div>Mention requirements (registration, fees, items to bring).</div>
                  <div>Upload a clear poster or relevant images.</div>
                </CardContent>
                <CardFooter>
                  <div className="text-xs text-muted-foreground">Event data is stored in your browser.</div>
                </CardFooter>
              </Card>
            </div>
          </div>
